...
resource "aws_opensearch_domain_policy" "main" {
  count = var.create && var.enable_access_policy ? 1 : 0

  domain_name     = aws_opensearch_domain.this[0].domain_name
  access_policies = var.create_access_policy ? data.aws_iam_policy_document.service[0].json : var.access_policies
}
...
data "aws_iam_policy_document" "service" {
  ...
  dynamic "statement" {
    for_each = var.access_pollicy_statements
